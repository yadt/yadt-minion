...
# Run this program to get an initial setup that allows you to run unit+integration tests.
...
if type apt-get &>/dev/null ; then
    dpkg -s $DEBIAN_PACKAGES python-virtualenv &>/dev/null || sudo apt-get install $DEBIAN_PACKAGES python-virtualenv
else
    rpm -q $REDHAT_PACKAGES python-virtualenv &>/dev/null || sudo yum install $REDHAT_PACKAGES python-virtualenv
fi
